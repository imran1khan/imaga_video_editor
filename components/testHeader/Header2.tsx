...
import { Circle, CircleEllipsis, Diamond, Eraser, Hand, Image as Img, Lock, Minus, MousePointer, MoveRight, Pen, PencilRuler, Square, Type } from 'lucide-react'
...
function Header2() {
    ...
    const [showOp,setShowOption]=useState(true);
    return (
        <header onMouseEnter={() => { mouseEnter() }} className="w-full flex justify-between absolute px-4 mt-2">
            ...
            <div className="dark:bg-gray-900 bg-slate-400 text-white rounded-md z-[1] flex items-center">
                <div className='flex items-center space-x-4 px-4 h-full'>
                    ...
                    <div onClick={() => {setShowOption(p=>!p)}} className={`relative hover:cursor-pointer h-full px-3 rounded-md flex items-center 
                        ${squareAtom === 'arc'|| squareAtom ==='ellipse' ? `dark:bg-purple-500 bg-green-600` : ``}`}>
                        <Circle size={15} strokeWidth={1.5} />
                        <div hidden={showOp} className='absolute top-10 left-2 w-auto h-auto bg-gray-950 rounded-md flex-col justify-center gap-2 px-3 py-2'>
                            <div onClick={() => {onClick('arc') }} className={`flex items-center gap-2 rounded-md 
                                ${squareAtom === 'arc' ? `dark:bg-purple-500 bg-green-600` : ``}`}>
                            <Circle size={15} strokeWidth={1.5} /><span>circle</span></div>
                            <div onClick={() => {onClick('ellipse') }} className={`flex items-center gap-2 rounded-md 
                                ${squareAtom === 'ellipse' ? `dark:bg-purple-500 bg-green-600` : ``}`}>
                                    <CircleEllipsis size={15} strokeWidth={1.5} /><span>ellipse</span>
                            </div>
                        </div>
                    </div>
